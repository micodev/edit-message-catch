...
function handle_edited_message(msg)
   msg.message_id = msg.message_id
   msg.rdate = msg.date
   msg.date = msg.edit_date
   msg.text = msg.text
   msg.from.id = msg.from.id
   msg.chat.type  = "edited"
   print(msg.from.id, msg.date,msg.text,msg.message_id,msg.rdate)
   	return msg_processor(msg)
 end
...
while is_running do
	...
	if response then
     ...
     handle_edited_message(v.edited_message)
			end
		end
	else
